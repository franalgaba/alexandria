...
import { ProgressiveRetriever, recommendLevel, getLevelConfig } from '../../src/retriever/progressive.ts';
...
describe('ProgressiveRetriever', () => {
  ...
  describe('getDeepContext', () => {
    test('has larger token budget', async () => {
      ...
      expect(pack.metadata?.tokenBudget).toBe(1500);
    });
...
describe('Level Utilities', () => {
  ...
  test('getLevelConfig returns correct budgets', () => {
    expect(getLevelConfig('minimal').tokenBudget).toBe(200);
    expect(getLevelConfig('task').tokenBudget).toBe(500);
    expect(getLevelConfig('deep').tokenBudget).toBe(1500);
  });
});
