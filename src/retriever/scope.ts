...
const FILE_PATH_PATTERN = /(?:in\s+|file\s+|from\s+)?([a-zA-Z0-9_\-./]+\.(ts|js|tsx|jsx|py|rs|go|java|rb|cpp|c|h|md|json|yaml|yml|toml|sql))/gi;

// Pattern for module/directory references
const MODULE_PATTERN = /(?:in\s+(?:the\s+)?|the\s+)([a-zA-Z0-9_-]+)\s+(?:module|directory|folder|package|component|service)/gi;
...
const KNOWN_AREAS = [
  ...
  'api', 'rest', 'graphql',
  'database', 'db', 'storage', 'store', 'stores',
  ...
  'middleware', 'handlers', 'controllers',
];
 ...
 */
export function codeRefsMatchScope(
  codeRefPaths: string[],
  queryScope: Scope
): number {
