...
function findIntegrationsDir(): string | null {
  ...
  const devPath = join(dirname(import.meta.path.replace('file://', '')), '..', '..', '..', 'integrations');
...
function installClaudeCode(integrationsDir: string, force: boolean): boolean {
  ...
  console.log('  • SessionStart hook for stale memory revalidation');
  console.log('  • Slash commands: /mem-search, /mem-add, /mem-pack, /mem-review');
...
export async function handler(argv: ArgumentsCamelCase<InstallArgs>): Promise<void> {
  console.log(colorize(argv.uninstall ? 'Alexandria Integration Uninstaller' : 'Alexandria Integration Installer', 'bold'));
  console.log();
  ...
  if (success) {
    console.log(colorize('Done!', 'green'), 'Restart your coding agent to activate the integration.');
  }
}
